...
public class VersionPlugin extends AbstractPlugin {
     ...
     * 
     * If a new version is created and be processed the plugin adds the attribute
     * '_versionprocessing' = true to indicate this situation for other plugins.
     * Other plugins can determine the $processingversion mode by calling the method
    ...
    public ItemCollection run(ItemCollection adocumentContext, ItemCollection adocumentActivity)
        ...
        try {
            ...
            if ("2".equals(adocumentActivity.getItemValueString("keyVersion"))) {
                ...
                /*
                 * this code iterates over all existing workitems with the same $workitemid and
                 * fixes lost parent workitemRefIDs.
                 */
